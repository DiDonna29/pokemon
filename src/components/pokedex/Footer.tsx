...
export function Footer({ lang }: FooterProps) {
  ...
  return (
    <footer className="relative z-10 nav-glass border-t border-foreground/5 py-8 md:py-12 mt-0">
...
function SocialLink({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <motion.a 
      ...
      whileTap={{ scale: 0.95 }}
      className="glass px-4 md:px-6 h-10 md:h-12 rounded-2xl flex items-center gap-3 border-foreground/5 hover:border-primary/40 hover:bg-primary/5 transition-all group"
    >
      <div className="text-muted-foreground group-hover:text-primary transition-colors">
        {icon}
      </div>
      <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-muted-foreground group-hover:text-foreground">
        {label}
      </span>
    </motion.a>
  );
