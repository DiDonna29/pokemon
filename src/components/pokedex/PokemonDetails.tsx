...
import { Ruler, Weight, Shield, Swords, Zap, Heart, ShieldAlert, Target, ArrowLeft } from "lucide-react";
...
export function PokemonDetailsView({ pokemon, onClose, lang, isCaught, onToggleCaught }: PokemonDetailsViewProps) {
  ...
  return (
    <Dialog open={!!pokemon} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-[100vw] md:max-w-2xl p-0 overflow-hidden border-none bg-background shadow-2xl rounded-none md:rounded-[3rem] h-[100dvh] md:h-[85vh] md:max-h-[850px] [&>button]:hidden z-[110]">
        <AnimatePresence>
          {pokemon && (
            <motion.div 
              initial={{ y: "100%" }}
            ...
            >
              <div className={cn(
                ...
                <div className="absolute top-8 left-0 right-0 px-6 flex items-center justify-between z-50">
                  <Button 
                    onClick={onClose}
                    className="glass h-12 px-6 rounded-2xl flex items-center justify-center gap-2 border-white/20 text-white hover:bg-white/20"
                  >
                  ...
                  <Button
                    ...
                    className={cn(
                      "glass h-16 w-16 rounded-2xl flex items-center justify-center border-white/20 transition-all duration-300 shadow-xl",
                  ...
                  >
                    <div className="relative w-10 h-10">
                      <Image 
                        ...
                        alt="Caught"
                        fill
                        className={cn("object-contain pixelated", isCaught ? "" : "grayscale opacity-40")}
                      />
                    </div>
                  </Button>
                </div>

                <div className="relative z-10 w-full flex items-center justify-center h-full pt-10">
                   <div className="absolute inset-0 bg-white/20 blur-[120px] rounded-full scale-110 opacity-60" />
                   {artwork && (
                    <Image 
                      src={artwork} 
                      alt={pokemon.name} 
                      width={400} 
                      height={400} 
                      className="relative z-10 drop-shadow-[0_40px_60px_rgba(0,0,0,0.6)] animate-float w-auto h-full max-h-[250px] md:max-h-[400px]"
                      priority
                    />
                  )}
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-6 md:px-12 pb-32 scrollbar-none">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {pokemon.types.map(typeInfo => (
                        <Badge 
                          ...
                          className={cn(
                            "px-6 py-2 rounded-full text-white font-black uppercase text-[10px] tracking-widest border-none shadow-xl",
                  ...
                  <Tabs defaultValue="about" className="w-full">
                    ...
                    <TabsContent value="about" className="space-y-10 outline-none">
                      <p className="text-muted-foreground leading-relaxed text-base md:text-lg font-medium italic border-l-4 border-primary/40 pl-6 py-2">
                        "{flavorText}"
                      </p>
                      
                      <div className="grid grid-cols-2 gap-6">
                        <div className="glass bg-foreground/5 rounded-[2.5rem] p-8 flex flex-col items-center gap-4 border-foreground/5">
                          <Ruler className="w-6 h-6 text-blue-500" />
                          <div className="text-center">
                            <div className="text-2xl md:text-3xl font-black">{pokemon.height / 10}m</div>
                            <div className="text-muted-foreground text-[10px] uppercase font-black tracking-widest opacity-40">{t.height}</div>
                          </div>
                        </div>
                        <div className="glass bg-foreground/5 rounded-[2.5rem] p-8 flex flex-col items-center gap-4 border-foreground/5">
                          <Weight className="w-6 h-6 text-orange-500" />
                          <div className="text-center">
                            <div className="text-2xl md:text-3xl font-black">{pokemon.weight / 10}kg</div>
                            <div className="text-muted-foreground text-[10px] uppercase font-black tracking-widest opacity-40">{t.weight}</div>
                          </div>
                        </div>
                      </div>
                    </TabsContent>
                      ...
                      {pokemon.stats.map((s, idx) => (
                        ...
                        >
                          <div className="flex items-center justify-between text-[11px] font-black uppercase tracking-widest">
                            ...
                            <span className="text-foreground font-black">{s.base_stat}</span>
                          </div>
                          <div className="relative h-2 w-full bg-foreground/5 rounded-full overflow-hidden">
                             <motion.div 
                               ...
                               className={cn(
                                 ...
                                 s.base_stat > 130 ? "bg-accent" : s.base_stat > 90 ? "bg-primary" : s.base_stat > 50 ? "bg-orange-500" : "bg-destructive"
                               )}
                             />
                          </div>
                        </motion.div>
